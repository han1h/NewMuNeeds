...
<!-- 메뉴바 -->
      ...
      <div class="mc1inlineContent">
   	...
   	<div id="layBodyWrap" class="mytmall_bodywrap">
   		...
   		<div class="mytmall_subArea_v2">
   			...
   			<!-- 비밀번호 입력 -->
   			<div class="password_recheck" style="width: 70%;  position: relative; top: 20%;  left: 20%;">
   				<form name="pwchkform" id="pwchkform">
   					<p class="password_info">
   						회원님의 개인정보를 안전하게 보호하기 위해
   						<br/>
   						<em class="fc_red">인증 후 변경이 가능</em>
   						합니다.
   					</p>
   					<div class="password_recheck_inner open">
   						<span class="in_btn">
   							MuNeeds 아이디:
   							<strong>${userVO.id }</strong>
   						</span>
   						<div class="password_box">
	   							<div class="inp_box" style="padding-bottom: 10px;">
	   								<label for="memScrtNo" class="password_lab">비밀번호를 입력하세요.</label>
			   					</div>
   							<p id="messageArea" class="err_msg" style="display:none;"></p>
   						</div>
   					</div>
   				</form>
				<div >
					<form action="gouser.inc" method="post" id="pwuser" name="pwuser" onsubmit="return check()"> 
						<input type="hidden" name="id" id="id" value="${userVO.id }">
						<input type="password" id="pw" class="password_inp" title="비밀번호 입력" name="pw" style="width: 30%;" >
					</form>
					<input type="submit" onclick="chkConfirmPassword()" value="확인" id="check"/>
				</div>
   			</div>
   			</c:if>
   			<c:if test="${ !empty sessionScope.userVO.nickname and sessionScope.userVO.status eq 2}">
   				<div style=" position: relative;  top: 190px;  text-align: center;">
   					<span>MuNeeds 로그인만 정보 변경 가능합니다.</span>
   				</div>
   			</c:if>
   		</div>
   	</div>
   ...
   <script type="text/javascript">
   		...
   		function chkConfirmPassword(){
   			...
   			if(${userVO.pw}== pw){
	   				document.pwuser.action = "gouser.inc";
	   				document.pwuser.submit();
   			}else{
   				alert("비밀번호가 다릅니다.");
   				$("#memScrtNo").val("");
   				$("#memScrtNo").focus();
